...
ATTR-1 (twelve confirmed attribution repairs through the F-15 channel) is AUTHORED, NOT EXECUTED — evidence population closed; migration/validation/rollback/harness authored (see /docs/ATTR-1_CONFIRMED_ATTRIBUTION_REPAIRS.md). `artist_id_override` NULL on all twelve, full external provenance, its own review and its own separate execution approval. Expected visible effect: twelve corrected illustrators and exactly one membership change (sui 224 → 223).
