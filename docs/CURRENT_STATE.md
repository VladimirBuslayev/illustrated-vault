...
Deferred by CAT-0: Pokémon TCG API fallback Probe B · broad image remediation (now CLOSED — the only authorized population was activated by CAT-3B.1; the remaining 1,448 gaps are not authorized for another broad repair phase, though IMG-0 / any future narrow evidence-backed image recovery remains a distinct, separate item in the Catalog Trust sequence, see ROADMAP.md) · illustrator enrichment · artist FK repair · OL-0D p_artist_id filter semantics (latent; no user-facing Owned Library artist-filter workflow established) · add_artist_to_archive duplicate-identity risk · index hygiene · pricing.
